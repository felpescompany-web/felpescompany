...
export function mountWormhole({ canvas }) {
  ...
  function play({ onComplete } = {}) {
    ...
    tl.to({}, { duration: 0.0 });
    if (horse) {
      tl.to(horse.position, { x: 0, y: 0, z: 0.5, duration: 1.2, ease: 'power2.in' }, 0)
        .to(horse.rotation, { y: 0, duration: 1.2, ease: 'power2.in' }, 0)
        .to(horse.position, { y: '+=0.10', repeat: 6, yoyo: true, duration: 0.1, ease: 'sine.inOut' }, 0);
    }

    tl.to(uniforms.uProgress, { value: 1.0, duration: 1.6, ease: 'power2.inOut' }, 0.5);
